...
{
   comm_i[d].Bcast(sum,1,MPI_INTEGER,root);
}
...
{
   comm_i[d].Bcast(sum,n,MPI_CHAR,root);
}
...
{
   comm_i[d].Send(sum,n,MPI_DOUBLE_PRECISION,procto,tag);
}
...
{
   MPI::Status status;
   comm_i[d].Recv(sum,n,MPI_DOUBLE_PRECISION,procfrom,tag);
}
