...
PGrid::PGrid(Parallel *inparall) : d3db(inparall,control_mapping(),control_ngrid(0),control_ngrid(1),control_ngrid(2)), lattice()
{
   ...
   for (k1 = 0;        k1<= nxh; ++k1)
   {
      gx = k1*unitg(0,0) + k2*unitg(0,1) + k3*unitg(0,2);
      gy = k1*unitg(1,0) + k2*unitg(1,1) + k3*unitg(1,2);
      gz = k1*unitg(2,0) + k2*unitg(2,1) + k3*unitg(2,2);
      i=k1; if (i < 0) i = i + nx;
      j=k2; if (j < 0) j = j + ny;
      k=k3; if (k < 0) k = k + nz;
