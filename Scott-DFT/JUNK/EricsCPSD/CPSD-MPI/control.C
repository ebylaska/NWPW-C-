...
static int bo_steps[2],bo_algorithm;
static int loop[2],ngrid[3],npsp,ncut,mapping,mapping1d;
static int np_dimensions[3],ewald_grid[3];
static int code,gga;
static int ispin,multiplicity;
static int move,frac_coord,SA,fei,fei_quench,gram_schmidt;
static int rotation,translation,balance,spin_orbit;
static int maxit_orb,maxit_orbs,scf_algorithm,ks_algorithm;
static int symm_number;
...
int control_balance()          { return balance; }
int control_mapping()          { return mapping; }
int control_np_orbital()       { return np_dimensions[1]; }
int control_ngrid(const int i) { return ngrid[i]; }
