...
)
{
   ...
   dp = (FORCETIME) ? NULL : ReadResultsFile(0, fnam);
   if (!dp)
   {
 ...
 */
   if (mmp->ID == 0 && !mmp->genstr && mmp->iftch > 0)
      FillInGMMNode(verb, mmp, pre, mmp->muladd, mmp->lat, beta, nb,
                    mmp->mu, mmp->nu, mmp->ku,
                    mmp->fftch, mmp->iftch, mmp->nftch,
                    FLAG_IS_SET(mmp->flag, MMF_LDCTOP), mmp->pref);
