 ...
 */
   ...
   fprintf(fpout, "#  ----------------------------\n");
   fprintf(fpout, "#  Make.inc for ATLAS3.9.23\n");
   fprintf(fpout, "#  ----------------------------\n\n");
 ...
 */
   if (ISAX == ISA_AV && CompIsGcc(comps[DMC_]) && !CompIsAppleGcc(comps[DMC_]))
   ...
   if (THREADS)
   {
      ...
      else if (omp) fprintf(fpout, " -DATL_TRUST_OMP");
      if (AntThr > 1) fprintf(fpout, " -DATL_ANTOINE_THREADS");
      else if (omp > 1) fprintf(fpout, " -DATL_OMP_THREADS");
   }
