...
#endif
#endif
static void ATL_sJIK0x0x64TT1x1x64_aX_bX
...
/*
 * matmul with TA=T, TB=T, MB=0, NB=0, KB=64, 
 ...
 */
{
   ...
   #define incAk 64
   const int incAm = ((lda) - 64), incAn = -(Mb*lda);
   ...
   #undef Kb
#endif
static void ATL_sJIK0x0x64TT4x1x64_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=T, MB=0, NB=0, KB=64, 
 ...
 */
{
   ...
   const float BetaAlpha = beta / alpha;
   #define incAk 64
   const int incAm = ((((lda) << 2)) - 64), incAn = -(Mb*lda);
   const int incBk = (ldb), incBm = -(((ldb) << 6));
   #define incBn 1
   #define incCm 4
   const int incCn = (ldc) - (Mb);
   ...
   if (k=M-Mb)
      ATL_sJIK0x0x64TT1x1x64_aX_bX(k, N, 64, alpha, ca + (Mb*lda), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
...
#endif
#endif
static void ATL_sJIK0x0x64TT1x2x64_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=T, MB=0, NB=0, KB=64, 
 ...
 */
{
   ...
   if (k=N-Nb)
      ATL_sJIK0x0x64TT1x1x64_aX_bX(M, k, 64, alpha, ca, lda, cb + (Nb), ldb, beta, cc + (Nb*ldc), ldc);
}
#ifdef incAm
   #undef incAm
#endif
   ...
   #undef Kb
#endif
void ATL_sJIK0x0x64TT0x0x0_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=T, MB=0, NB=0, KB=64, 
 ...
 */
{
   ...
   if (k=N-Nb)
      ATL_sJIK0x0x64TT4x1x64_aX_bX(M, k, 64, alpha, ca, lda, cb + (Nb), ldb, beta, cc + (Nb*ldc), ldc);
   if (Nb && (k=M-Mb))
      ATL_sJIK0x0x64TT1x2x64_aX_bX(k, Nb, 64, alpha, ca + (Mb*lda), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
