 ...
 */
{
   ...
   #undef Kb
#endif
static void ATL_sJIK0x0x17TN4x1x17_a1_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=17, 
...
#endif
#endif
static void ATL_sJIK0x0x17TN1x2x17_a1_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=17, 
 ...
 */
{
   ...
   if (k=N-Nb)
      ATL_sJIK0x0x17TN1x1x17_a1_bX(M, k, 17, alpha, ca, lda, cb + (((Nb) << 4)+Nb), ldb, beta, cc + (Nb*ldc), ldc);
}
#ifdef incAm
   #undef incAm
#endif
 ...
 */
{
   ...
   if (k=N-Nb)
      ATL_sJIK0x0x17TN4x1x17_a1_bX(M, k, 17, alpha, ca, lda, cb + (((Nb) << 4)+Nb), ldb, beta, cc + (Nb*ldc), ldc);
   if (Nb && (k=M-Mb))
      ATL_sJIK0x0x17TN1x2x17_a1_bX(k, Nb, 17, alpha, ca + (((Mb) << 4)+Mb), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
