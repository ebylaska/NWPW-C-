 ...
 */
{
   ...
   #undef Kb
#endif
static void ATL_sJIK0x0x10TN4x1x10_a1_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=10, 
...
#endif
#endif
static void ATL_sJIK0x0x10TN1x2x10_a1_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=10, 
 ...
 */
{
   ...
   if (k=N-Nb)
      ATL_sJIK0x0x10TN4x1x10_a1_bX(M, k, 10, alpha, ca, lda, cb + (((Nb) << 3)+((Nb) << 1)), ldb, beta, cc + (Nb*ldc), ldc);
   if (Nb && (k=M-Mb))
      ATL_sJIK0x0x10TN1x2x10_a1_bX(k, Nb, 10, alpha, ca + (((Mb) << 3)+((Mb) << 1)), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
