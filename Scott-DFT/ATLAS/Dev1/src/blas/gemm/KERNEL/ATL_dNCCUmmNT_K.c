...
#endif
#endif
static void ATL_dJIK0x0x48NT1x1x24_aX_bX
...
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=48, 
 ...
 */
   ...
   #undef Kb
#endif
static void ATL_dJIK0x0x48NT2x1x24_aX_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=48, 
 ...
 */
{
   ...
   const double BetaAlpha = beta / alpha;
   const int incAk = (lda);
   const int incAm = 2 - (((lda) << 5)+((lda) << 4)), incAn = -(Mb);
   const int incBk = (ldb), incBm = -(((ldb) << 5)+((ldb) << 4));
   #define incBn 1
   #define incAk0 incAk
   #define incBk0 incBk
 ...
 */
   ...
   if (k=M-Mb)
      ATL_dJIK0x0x48NT1x1x24_aX_bX(k, N, 48, alpha, ca + (Mb), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
...
#endif
#endif
static void ATL_dJIK0x0x48NT1x2x24_aX_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=48, 
 ...
 */
   ...
   if (k=N-Nb)
      ATL_dJIK0x0x48NT1x1x24_aX_bX(M, k, 48, alpha, ca, lda, cb + (Nb), ldb, beta, cc + (Nb*ldc), ldc);
}
#ifdef incAm
   #undef incAm
#endif
   ...
   #undef Kb
#endif
void ATL_dJIK0x0x48NT0x0x0_aX_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=48, 
 ...
 */
   ...
   if (k=N-Nb)
      ATL_dJIK0x0x48NT2x1x24_aX_bX(M, k, 48, alpha, ca, lda, cb + (Nb), ldb, beta, cc + (Nb*ldc), ldc);
   if (Nb && (k=M-Mb))
      ATL_dJIK0x0x48NT1x2x24_aX_bX(k, Nb, 48, alpha, ca + (Mb), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
