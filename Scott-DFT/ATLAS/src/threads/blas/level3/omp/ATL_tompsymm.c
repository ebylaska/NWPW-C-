 ...
 */
   ...
   B10 = B + (nL SHIFT);
   C10 = C + (nL SHIFT);
   ATL_ompsymmL_rec(syp, nbL, rL, Nblks, nr, A, B, C);
   ATL_ompsymmL_rec(syp, nbR, rR, Nblks, nr, A+(syp->lda+1)*(nL SHIFT), B10, C10);
   if (syp->uplo == AtlasLower)
   {
      A10 = A + (nL SHIFT);
      Mjoin(PATL,tompgemm)(AtlasTrans, AtlasNoTrans, nL, syp->N, nR,
   ...
   else
   {
      ...
      Mjoin(PATL,tompgemm)(AtlasNoTrans, AtlasNoTrans, nL, syp->N, nR,
                        ...
                        ONE, C, syp->ldc);
      Mjoin(PATL,tompgemm)(AtlasTrans, AtlasNoTrans, nR, syp->N, nL,
                        SVVAL((TYPE*)syp->alpha), A01, syp->lda, B, syp->ldb,
                        ONE, C10, syp->ldc);
   }
}
static void ATL_ompsymmR_rec
   (ATL_TSYMM_t *syp, ATL_CINT Mblks, ATL_CINT mr, ATL_CINT Nblks, ATL_CINT nr,
    const TYPE *A, const TYPE *B, TYPE *C)
{
 ...
 */
   ...
   ATL_ompsymmL_rec(syp, Mblks, mr, nbL, rL, A, B, C);
   ATL_ompsymmL_rec(syp, Mblks, mr, nbR, rR, A+(syp->lda+1)*(nL SHIFT), B01, C01);
   if (syp->uplo == AtlasLower)
   {
      A10 = A + (nL SHIFT);
      Mjoin(PATL,tompgemm)(AtlasNoTrans, AtlasNoTrans, syp->M, nL, nR,
                        ...
                        ONE, C, syp->ldc);
      Mjoin(PATL,tompgemm)(AtlasNoTrans, AtlasTrans, syp->M, nR, nL,
   ...
   {
      A01 = A + (syp->lda SHIFT);
      Mjoin(PATL,tompgemm)(AtlasNoTrans, AtlasTrans, syp->M, nL, nR,
...
static void ATL_tompsymm_SYsplit
 ...
 */
{
   ...
   ss.ldc = ldc;
   if (Side == AtlasLeft)
      ATL_ompsymmL_rec(&ss, M/nb, M%nb, N/nb, N%nb, A, B, C);
   else
      ATL_ompsymmR_rec(&ss, M/nb, M%nb, N/nb, N%nb, A, B, C);
 ...
 */
         ...
         {
            ATL_tompsymm_SYsplit(Side, Uplo, M, N, alpha, A, lda, B, ldb,
                              beta, C, ldc, nb);
            return;
         }
      }
...
SERIAL:
   ...
   ls.OpStructIsInit = Mjoin(PATL,StructIsInitSYMM);
   ls.DoWork = Mjoin(PATL,DoWorkSYMM);
   ls.rank2thr = tp;
   /*
