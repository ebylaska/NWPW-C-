...
)
{
   ...
   if (Uplo == AtlasLower)
   {
      ...
      for (k=0; k < p; k++)
      {
         ...
         syp[j].M = m;
         syp[j].N = n;
         syp[j].T = MindxT(C,(n*incC));
         syp[j].C = (n > 0) ? MindxT(C,(n<<eltsh)) : NULL;
         syp[j].A0 = (ISNOTRANS) ? MindxT(A,(n<<eltsh)) : MindxT(A,n*incA);
         syp[j].A = syp[j].A0;
         syp[j].B = A;
         n += m;
      }
   ...
   else  /* Uplo == AtlasUpper */
   {
      ...
      for (k=0; k < p; k++)
      {
         ...
         syp[j].N = n;
         JJ = N - n - m;
         syp[j].T = MindxT(C,(JJ*incC));
         syp[j].C = (n > 0) ? MindxT(C,(JJ*incC+m*(ldc<<eltsh))) : NULL;
         if (ISNOTRANS)
         {
            syp[j].A = syp[j].A0 = MindxT(A,(JJ<<eltsh));
            syp[j].B = MindxT(syp[j].A0, (m<<eltsh));
         }
         else
         {
            syp[j].A = syp[j].A0 = MindxT(A,(JJ*incA));
            syp[j].B = MindxT(syp[j].A0, (m*incA));
         }
         n += m;
      }
   }
