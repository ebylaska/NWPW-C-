...
{
   ATL_TSYMM_t *sp=vp;
   Mjoin(PATL,hemm)(sp->side, sp->uplo, sp->M, sp->N, SVVAL((TYPE*)sp->alpha),
                     sp->A, sp->lda, sp->B, sp->ldb, SVVAL((TYPE*)sp->beta),
                     sp->C, sp->ldc);
}

static void ATL_hemmL_rec
 ...
 */
   ...
   B10 = B + (nL SHIFT);
   C10 = C + (nL SHIFT);
   ATL_hemmL_rec(syp, nbL, rL, Nblks, nr, A, B, C);
   ATL_hemmL_rec(syp, nbR, rR, Nblks, nr, A+(syp->lda+1)*(nL SHIFT), B10, C10);
   if (syp->uplo == AtlasLower)
   {
      A10 = A + (nL SHIFT);
      Mjoin(PATL,tgemm)(AtlasConjTrans, AtlasNoTrans, nL, syp->N, nR,
   ...
   else
   {
      ...
      Mjoin(PATL,tgemm)(AtlasNoTrans, AtlasNoTrans, nL, syp->N, nR,
                        ...
                        ONE, C, syp->ldc);
      Mjoin(PATL,tgemm)(AtlasConjTrans, AtlasNoTrans, nR, syp->N, nL,
                        SVVAL((TYPE*)syp->alpha), A01, syp->lda, B, syp->ldb,
                        ONE, C10, syp->ldc);
   }
}
static void ATL_hemmR_rec
   (ATL_TSYMM_t *syp, ATL_CINT Mblks, ATL_CINT mr, ATL_CINT Nblks, ATL_CINT nr,
    const TYPE *A, const TYPE *B, TYPE *C)
{
 ...
 */
   ...
   ATL_hemmL_rec(syp, Mblks, mr, nbL, rL, A, B, C);
   ATL_hemmL_rec(syp, Mblks, mr, nbR, rR, A+(syp->lda+1)*(nL SHIFT), B01, C01);
   if (syp->uplo == AtlasLower)
   {
      A10 = A + (nL SHIFT);
      Mjoin(PATL,tgemm)(AtlasNoTrans, AtlasNoTrans, syp->M, nL, nR,
                        ...
                        ONE, C, syp->ldc);
      Mjoin(PATL,tgemm)(AtlasNoTrans, AtlasConjTrans, syp->M, nR, nL,
   ...
   {
      A01 = A + (syp->lda SHIFT);
      Mjoin(PATL,tgemm)(AtlasNoTrans, AtlasConjTrans, syp->M, nL, nR,
 ...
 */
{
   ...
   ss.ldc = ldc;
   if (Side == AtlasLeft)
      ATL_hemmL_rec(&ss, M/nb, M%nb, N/nb, N%nb, A, B, C);
   else
      ATL_hemmR_rec(&ss, M/nb, M%nb, N/nb, N%nb, A, B, C);
 ...
 */
         ...
         {
            ATL_themm_SYsplit(Side, Uplo, M, N, alpha, A, lda, B, ldb,
                              beta, C, ldc, nb);
            return;
         }
      }
