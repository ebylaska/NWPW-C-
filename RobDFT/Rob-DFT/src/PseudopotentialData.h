...
class PseudoPotentialData {
 public:
  ...
  vector< thrust::host_vector<Real> > Gijl;
  vector< thrust::host_vector<Real> > vl;
  vector< thrust::host_vector<Real> > vnl;
  ...
  PseudoPotentialData(char* rootFilename, Ion* myIon, int nfft3d)
	...
	Gijl.push_back(thrust::host_vector<Real>(nn));
	buf += _bRead(buf,&Gijl[ia][0], nn); 
	...
	if (nprj[ia] > 0) {
	  ...
	  vnl.push_back(thrust::host_vector<Real>(nprj[ia]*nfft3d));
	  buf += _bRead(buf,&vnl[ia][0], nprj[ia]*nfft3d); 
          cerr << " vnl1 =" << vnl[ia][0] << " " << vnl[ia][1] << endl;
          cerr << " vnl2 =" << vnl[ia][nfft3d] << " " << vnl[ia][nfft3d+1] << endl;
          cerr << " vnl3 =" << vnl[ia][2*nfft3d] << " " << vnl[ia][2*nfft3d+1] << endl;
          cerr << " vnl4 =" << vnl[ia][3*nfft3d] << " " << vnl[ia][3*nfft3d+1] << endl;
	}
	if (semicore[ia] > 0) {
