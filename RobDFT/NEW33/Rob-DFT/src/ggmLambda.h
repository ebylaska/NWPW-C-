...
inline int myIdamax(int n, Real* x)
{
  int ret=0;
  Real m=x[0];
  for(int i=1; i < n; i++) if(m < x[i]) { m = x[i]; ret=i; }
...
inline void m_scale_s22(const int mb, const Real dte, Real *s22)
{
...
inline void m_scale_s21(const int mb, const Real dte, Real *s21)
{
...
inline void m_scale_s11(const int mb, const Real dte, Real *s11)
{
  int ms1,ms2,ishift2,indx0,indx,indxt;
  if (mb==-1) {   ms1=0; ms2=psiHeader.get_ispin(); ishift2=psiHeader.get_ne(0)*psiHeader.get_ne(0);}
  else {   ms1=mb; ms2=mb+1; ishift2=0;}
...
void ggm_lambda()
{
  ...
  thrust::host_vector<Real> h_lambda(7*psiHeader.get_ne(0)*psiHeader.get_ne(0));
