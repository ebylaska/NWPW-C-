...
void ggm_lambda()
{
  ...
  for (int ms=0; ms < psiHeader.get_ispin(); ++ms) {
    ...
    // neall is equal to (psiHeader.get_ne(0) + psiHeader.get_ne(1)
    ...
    int offset = ms * psiHeader.get_ne(ms) * cd->n2ft3d;
    blasGemm('T', 'N',  psiHeader.get_ne(ms), psiHeader.get_ne(ms), cd->n2ft3d, (Real) 1.,
	     (Real*) thrust::raw_pointer_cast(&psi_r[offset]), cd->n2ft3d,
	     (Real*) thrust::raw_pointer_cast(&(*psi2)[offset]), cd->n2ft3d, (Real) 0.,
	     (Real*) thrust::raw_pointer_cast(&s22[0]), 
	     psiHeader.get_ne(ms));
    h_s22 = s22;
    //for(int indx=0; indx < 6*6; indx++) cout << "s22 " << h_s22[indx] << endl;
    ...
    blasGemm('T', 'N', psiHeader.get_ne(ms), psiHeader.get_ne(ms), cd->n2ft3d, (Real) 1.,
	     ...
	     (Real*) thrust::raw_pointer_cast(&s21[0]), 
	     psiHeader.get_ne(ms));
    h_s21 = s21;
    //for(int indx=0; indx < 6*6; indx++) cout << "s21 " << h_s21[indx] << endl;
    ...
    blasGemm('T', 'N', psiHeader.get_ne(ms), psiHeader.get_ne(ms), cd->n2ft3d, (Real) 1.,
	     (Real*) thrust::raw_pointer_cast(&psi_r[offset]), cd->n2ft3d,
	     (Real*) thrust::raw_pointer_cast(&(*psi1)[offset]), cd->n2ft3d, (Real) 0.,
	     (Real*) thrust::raw_pointer_cast(&s11[0]), 
	     psiHeader.get_ne(ms));
    h_s11 = s11;
    //normCheck();
    cout << "s11 " ;
