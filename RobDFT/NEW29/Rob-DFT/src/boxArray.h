 ...
 *                                                *
 **************************************************/
/* returns the maximum of the diagonal of f'f

*/
  struct ff_sum_functor {
     const Real2* psi1;
     const Real* summer;

     ff_sum_functor(const Real2* _psi1,
                    const Real* _summer)
                    : psi1(_psi1),
                      summer(_summer) {};
    ...
    {
      register Real tmp  = summer[tid];
      register Real2 p1  = psi1[tid];
      Real sum1 = (p1.x*p1.x + p1.y*p1.y)*tmp;
      return(sum1);
    }
  };
inline Real f_Max_Diag(const thrust::device_vector<Real2> &d_v1)
{
   Real sum1 = 0.0;
   Real sumt;
   for (int i=0; i<neall; ++i)
   {
      ff_sum_functor my_op(thrust::raw_pointer_cast(&d_v1[i*(cd->nfft3d)]),
                           thrust::raw_pointer_cast(&cd->d_summer[1][0]));
      sumt = thrust::transform_reduce(thrust::counting_iterator<unsigned int>(0),
                                      thrust::counting_iterator<unsigned int>(cd->nfft3d),
                                      my_op,
                                      (Real) 0.,
                                      thrust::plus<Real>());
     if (sumt>sum1) sum1=sumt;
   }
   return sum1;
}
 ...
 *                                           *
 *********************************************/
  struct d_daxpy_functor
  {
     const Real  a;
     const Real* vpsi;
     Real*       hpsi;

    d_daxpy_functor(const Real _a,
                    const Real *_vpsi,
                    Real* _hpsi) 
                   : a(_a), vpsi(_vpsi), hpsi(_hpsi) {};
    __device__
    void operator()(unsigned int i)
    {
      hpsi[i] += a * vpsi[i];
  ...
  void d_daxpy(int nsize,
               Real scal1,
               const thrust::device_vector<Real> &vpsi,
                     thrust::device_vector<Real> &hpsi)
  {
     d_daxpy_functor unary_op(scal1,
                              thrust::raw_pointer_cast(&vpsi[0]),
                              thrust::raw_pointer_cast(&hpsi[0]) );
     thrust::for_each(thrust::counting_iterator<unsigned int>(0),
                      thrust::counting_iterator<unsigned int>(nsize),
                      unary_op);
  }
  ...
  void inner_loop()
  {
    ...
    *psi1 = *psi2; Ke = kineticEnergy();
    ...
    for(int iter=0; iter < it_in; iter++) {
      ...
      cerr << "kspace_operator took " << (endTime - startTime) << " wall seconds" << endl;
	...
	for (int ms=0; ms < ispin; ++ms) {
      ...
      cerr << "ggm_lambda took      " << (endTime - startTime) << " wall seconds" << endl;

    } /* loop finished */
   Real endloop = omp_get_wtime();
   cerr << "cputime/step         " << (endloop - startloop)/((Real) it_in) << " wall seconds" << endl;

    Ke = normCheck(*psi2);
    cerr << "Final norm is " << Ke << endl;

    thrust::host_vector<Real>   h_hml(psiHeader.get_ne(0)*psiHeader.get_ne(0) +psiHeader.get_ne(1)*psiHeader.get_ne(1));
    thrust::device_vector<Real> d_hml = h_hml;

    ffm_Multiply(-1,*psi1,Hpsi,d_hml);
    h_hml = d_hml;
    for (int i=0; i<36; ++i) h_hml[i] = -h_hml[i];
    cout << "hml =" << endl;
    cout <<h_hml[0]<<" "<<h_hml[6]<<" " <<h_hml[12]<<" "<<h_hml[18]<<" "<<h_hml[24]<<" "<<h_hml[30]<<endl;
    cout <<h_hml[1]<<" "<<h_hml[7]<<" " <<h_hml[13]<<" "<<h_hml[19]<<" "<<h_hml[25]<<" "<<h_hml[31]<<endl;
    cout <<h_hml[2]<<" "<<h_hml[8]<<" " <<h_hml[14]<<" "<<h_hml[20]<<" "<<h_hml[26]<<" "<<h_hml[32]<<endl;
    cout <<h_hml[3]<<" "<<h_hml[9]<<" " <<h_hml[15]<<" "<<h_hml[21]<<" "<<h_hml[27]<<" "<<h_hml[33]<<endl;
    cout <<h_hml[4]<<" "<<h_hml[10]<<" "<<h_hml[16]<<" "<<h_hml[22]<<" "<<h_hml[28]<<" "<<h_hml[34]<<endl;
    cout <<h_hml[5]<<" "<<h_hml[11]<<" "<<h_hml[17]<<" "<<h_hml[23]<<" "<<h_hml[29]<<" "<<h_hml[35]<<endl<<endl;
    ...
    cout << "K.S. V_xc energy    = " << vxc   << endl << endl;

    cout << "psi error1          = " << deltac   << endl;

    startloop = omp_get_wtime();
    thrust::host_vector<Real2> h_psi1 = *psi1;
    *psi2 = h_psi1;
    endloop = omp_get_wtime();
    cerr << "devicetohost time  " << (endloop - startloop) << " wall seconds" << endl;
...
*/
  ...
  inline void myAxpy(int n, const Real alpha, Real *x, Real *y) {
  ...
  void v_nonlocal()
  {
    ...
    cerr << "number of atoms = " << (h_pseudoData.myIonPtr->nion) << endl;
	...
	/* structure factor */
	//h_strfac.strfac_exi(ii, h_exi); d_exi = h_exi;
...
#define dncut  1.0e-20
  ...
  void debugSum(thrust::device_vector<Real2> &a, const char* s) {
    thrust::host_vector<Real2> h_a = a;
    double sum=0.; for(int i=0; i < h_a.size(); i++) { sum += h_a[i].x; sum += h_a[i].y; }
    cerr << "DEBUG sum " << s << " " << sum << endl;
  }
  void debugSum(thrust::device_vector<Real> &a, const char* s) {
    thrust::host_vector<Real> h_a = a;
    double sum=0.; for(int i=0; i < h_a.size(); i++) sum += h_a[i];
    cerr << "DEBUG sum " << s << " " << sum << endl;
  }
  void debugSum(thrust::host_vector<Real> &h_a, const char* s) {
    double sum=0.; for(int i=0; i < h_a.size(); i++) sum += h_a[i];
    cerr << "DEBUG sum " << s << " " << sum << endl;
  }
  void debugSum(thrust::host_vector<Real2> &h_a, const char* s) {
    double sum=0.; for(int i=0; i < h_a.size(); i++) { sum += h_a[i].x; sum += h_a[i].y; }
    cerr << "DEBUG sum " << s << " " << sum << endl;
  }
  void debugSum(int offset, int n, thrust::host_vector<Real2> &h_a, const char* s) {
    double sum=0.; for(int i=0; i < n; i++) { sum += h_a[i+offset].x; sum += h_a[i+offset].y; }
    cerr << "DEBUG sum " << s << " " << sum << endl;
  }
  void debugSum(int offset, int n, const thrust::device_vector<Real2> &a, const char* s) {
    thrust::host_vector<Real2> h_a = a;
    double sum=0.; for(int i=0; i < n; i++) { sum += h_a[i+offset].x; sum += h_a[i+offset].y; }
    cerr << "DEBUG sum " << s << " " << sum << endl;
  }
  void debugSum(const int offset, const int n, const thrust::device_vector<Real> &a, const char* s) {
    thrust::host_vector<Real> h_a = a;
    double sum=0.; for(int i=0; i < n; i++) { sum += h_a[i+offset]; }
    cerr << "DEBUG sum " << s << " " << sum << endl;
  }
