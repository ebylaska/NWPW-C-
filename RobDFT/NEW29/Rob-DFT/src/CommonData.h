...
class CommonData {
 public:
  ...
  thrust::device_vector<Real> d_id;
  thrust::device_vector<int> d_zero_end;
  ...
  thrust::host_vector<int> h_zero_end;
  ...
  void precomputeVg()
  {
    ...
    cerr << "fourpi=" << fourpi << endl;
    h_vg = thrust::host_vector<Real>(nfft3d);
    
    h_vg[0] = 0.0;
    for(int k=1; k < nfft3d; ++k) {
      Real gg = G[0][k]*G[0][k] + G[1][k]*G[1][k] + G[2][k]*G[2][k];
      h_vg[k] = fourpi/gg;
    }
    for(int k=0; k < nfft3d; ++k) h_vg[k] *= h_masker[0][k];
    d_vg = h_vg;
  }
  ...
  void precompute_zero_end()
  {
    int i,j,k,indx;
    d_zero_end = thrust::device_vector<int>(n2ft3d);
    h_zero_end = thrust::host_vector<int>(n2ft3d);
  ...
  CommonData(PsiHeader<Real>& psiHeader, RTDB& myrtdb) {
    ...
    int msize = psiHeader.get_ne(0)*psiHeader.get_ne(0) 
              + psiHeader.get_ne(1)*psiHeader.get_ne(1);
    d_id = thrust::device_vector<Real>(msize);
    thrust::host_vector<Real> h_id(msize);

    for (int i=0; i<msize; ++i) 
       h_id[i] = (Real) 0.0;
  
    int nn = psiHeader.get_ne(0)*psiHeader.get_ne(0);
    for (int ms=0; ms<psiHeader.get_ispin(); ++ms)
    {
       int n = psiHeader.get_ne(ms);
       for (int i=0; i<n; ++i)
          h_id[i+i*n + ms*nn] = (Real) 1.0;
    }
    d_id = h_id;
