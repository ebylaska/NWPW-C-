...
inline int lsame_(char* a, char* b) { return(*a == *b); }

/* Subroutine */
 int Rmf_Dgemm(char *transa, char *transb, int *m, int *n, int *k, Real *alpha, Real *a, int *lda, 
    ...
    if (nota) {
      ...
      /*           Form  C := alpha*A'*B + beta*C */
        ...
        for (i = 1; i <= *m; ++i) {
          ...
          i__3 = *k;
          for (l = 1; l <= *k; ++l) {
	    cerr << "RMF_Dgemm A(" << l << "," << i <<") " << A(l,i) << " " << B(l,j) << endl;
            temp += A(l,i) * B(l,j);
            /* L100: */
          }
          if (*beta == 0.) {
