...
class boxArray {
 ...
 public:
  ...
  void inner_loop()
  {
    ...
    *psi1 = *psi2; Ke = kineticEnergy();
    ...
    for(int iter=0; iter < it_in; iter++) {
      ...
      //cerr << "NORM CHECK " << endl; normCheck( *psi1 );
	...
	cerr << "DEBUG***** dn=" << sum*((30.0*30.0*30.0)/(48.0*48.0*48.0)) << endl;
      ...
      thrust::device_vector<Real> d_xcp(2*ispin*cd->nfft3d);
      thrust::device_vector<Real> d_xce(2*ispin*cd->nfft3d); 
      {
  ...
  void checkNonlocal() {
    ...
    v_nonlocal(); 
    Ke = -2.0*psiHpsiCheck();
    cerr << "DEBUG CheckNonlocal Ke " << Ke << endl;
    //exit(1);
  }
