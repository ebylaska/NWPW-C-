...
c       iestimate = FFTW_ESTIMATE
   ...
   if (maptype==1) 
   {
       ...
       Atest = new double[2*nx*ny*nq];

       dfftw_plan_many_dft(plans(1),1,nz,nxh,
     ...
     >                        iforward,FFTW_EXHAUSTIVE)
         ...
         if (.not.MA_free_heap(Atest(2)))
     >   call errquit('D3dB_fft_init:freeing heap',0,MA_ERR)

      else

         nxh  = (nx(nb)/2+1)
         if (.not.MA_alloc_get(mt_dcpl,nfft3d(nb),'Atest',
     >                         Atest(2),Atest(1)))
      ...
      if (taskid==MASTER)
         for (int k=0; k<nz; ++k)
         {
            ...
            for (jj=jstart; jj<=jend; ++jj)
            {
               ...
               if (p_to==MASTER)
                  for (int i=0; i<bsize; ++i) a[index+i] = tmp[i];
               else
                  parall->dsend(0,9,p_to,bsize,tmp);
            }
         }
