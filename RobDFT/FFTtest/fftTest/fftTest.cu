...
static void test(int *dim, int neall)
{
  ...
  thrust::host_vector<Real> h_testData2(n2ft3d*neall);
  thrust::host_vector<Real> h_testData3(n2ft3d*neall);


  for(int fft=0; fft < neall; fft++)
    for(int k=0; k < n2ft3d; k++)
       h_testData[k+fft*n2ft3d] = k+fft*n2ft3d;
  ...
  d_testData  = h_testData;
  thrust::device_vector<Real> d_testData2 = h_testData2;
  thrust::device_vector<Real> d_testData3 = h_testData3;
  d_testData2 = d_testData;
  rcFFT_neall(thrust::raw_pointer_cast(&d_testData[0]),
	      thrust::raw_pointer_cast(&d_testData[0]));
  crFFT_neall(thrust::raw_pointer_cast(&d_testData[0]),
	      thrust::raw_pointer_cast(&d_testData2[0]));
  h_testData3 = d_testData2;
  ...
  for(int fft=0; fft < neall; fft++)
    for (int i=0; i<n2ft3d; ++i) 
    {
       ...
       h_testData3[i+fft*n2ft3d] /= (Real) nfft;
       cout << "test data " << h_testData1[i+fft*n2ft3d] << " should be " 
	       << h_testData[i+fft*n2ft3d] << "  outplace=" << h_testData3[i+fft*n2ft3d] << endl;
    }
}
